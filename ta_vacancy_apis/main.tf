...
# ------------------------------ Delete TA Vacancy List-------------------------------------------------
# ----------------------------------------------------------------------------------------------------

resource "aws_iam_role" "iam_delete_ta_vacancy_by_vac_id" {
  name = "iam_delete_ta_vacancy_by_vac_id_${var.env}"
...
resource "aws_iam_policy" "delete_ta_vacancy_by_vac_id_policy" {
  name        = "delete_ta_vacancy_by_vac_id_policy-${var.env}"
  description = "A ses policy"
...
resource "aws_iam_role_policy_attachment" "test_attach_delete_ta_vacancy_by_vac_id" {
  role       = aws_iam_role.iam_delete_ta_vacancy_by_vac_id.name
  policy_arn = aws_iam_policy.delete_ta_vacancy_by_vac_id_policy.arn
}


resource "aws_lambda_function" "delete_ta_vacancy_by_vac_id" {


  filename      = "../../ta_vacancy_apis/delete_ta_vacancy_by_vac_id.zip"
  function_name = "delete_ta_vacancy_by_vac_id"
  role          = aws_iam_role.iam_delete_ta_vacancy_by_vac_id.arn
  handler       = "lambda_function.lambda_handler"

  source_code_hash = filebase64sha256("../../ta_vacancy_apis/delete_ta_vacancy_by_vac_id.zip")
...
resource "aws_lambda_permission" "aws_lambda_delete_ta_vacancy_by_vac_id_permission" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.delete_ta_vacancy_by_vac_id.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn = "${aws_api_gateway_rest_api.ta_vacancy_form.execution_arn}/*/*"
}
...
# child resource : delete_ta_vacancy_by_vac_id
resource "aws_api_gateway_resource" "delete_ta_vacancy" {
  rest_api_id = aws_api_gateway_rest_api.ta_vacancy_form.id
  parent_id = aws_api_gateway_resource.ta_vacancy_form.id
  path_part = "delete_ta_vacancy"
}

# child resource : delete_ta_vacancy_by_vac_id
resource "aws_api_gateway_resource" "delete_ta_vacancy_by_vac_id" {
  rest_api_id = aws_api_gateway_rest_api.ta_vacancy_form.id
  parent_id = aws_api_gateway_resource.delete_ta_vacancy.id
  path_part = "{ta_vac_id}"
}

# delete_ta_vacancy_by_vac_id : method
resource "aws_api_gateway_method" "delete_ta_vacancy_by_vac_id" {
  rest_api_id = aws_api_gateway_rest_api.ta_vacancy_form.id
  resource_id = aws_api_gateway_resource.delete_ta_vacancy_by_vac_id.id
  http_method = "GET"
  authorization = "COGNITO_USER_POOLS"
  authorizer_id = aws_api_gateway_authorizer.ta_vacancy_form.id
}

#  lambda integration : delete_ta_vacancy_by_vac_id-post-lambda
resource "aws_api_gateway_integration" "delete_ta_vacancy_by_vac_id-post-lambda" {
  rest_api_id = aws_api_gateway_rest_api.ta_vacancy_form.id
  resource_id = aws_api_gateway_method.delete_ta_vacancy_by_vac_id.resource_id
  http_method = aws_api_gateway_method.delete_ta_vacancy_by_vac_id.http_method
  integration_http_method = "POST"
  type = "AWS_PROXY"
  uri = aws_lambda_function.delete_ta_vacancy_by_vac_id.invoke_arn
}



module "cors_delete_ta_vacancy_by_vac_id" {
  source  = "squidfunk/api-gateway-enable-cors/aws"
  version = "0.3.3"
  api_id          = aws_api_gateway_rest_api.ta_vacancy_form.id
  api_resource_id = aws_api_gateway_resource.delete_ta_vacancy_by_vac_id.id
}


resource "aws_api_gateway_deployment" "ta_vacancy_form_deployment" {
  depends_on= [aws_api_gateway_integration.add_ta_vacancy-post-lambda,aws_api_gateway_integration.get_ta_vacancy_list-post-lambda,aws_api_gateway_integration.get_ta_vacancy_list_by_user_id-post-lambda,aws_api_gateway_integration.delete_ta_vacancy_by_vac_id-post-lambda]
  rest_api_id = aws_api_gateway_rest_api.ta_vacancy_form.id
  stage_name  = "${var.env}"
}
