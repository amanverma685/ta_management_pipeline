...
# ---------------------------------------List of students applied for taship --------------------------------------------
# -----------------------------------------------------------------------------------------------------------------------

resource "aws_iam_role" "iam_list_of_student_applied_for_taship" {
  name = "iam_list_of_student_applied_for_taship_${var.env}"
...
resource "aws_iam_policy" "list_of_student_applied_for_taship_policy" {
  name        = "list_of_student_applied_for_taship_policy-${var.env}"
  description = "A ses policy"
...
resource "aws_iam_role_policy_attachment" "test_attach_list_of_student_applied_for_taship" {
  role       = aws_iam_role.iam_list_of_student_applied_for_taship.name
  policy_arn = aws_iam_policy.list_of_student_applied_for_taship_policy.arn
}


resource "aws_lambda_function" "list_of_student_applied_for_taship" {


  filename      = "../../student_request_for_taship/list_of_student_applied_for_taship.zip"
  function_name = "list_of_student_applied_for_taship"
  role          = aws_iam_role.iam_list_of_student_applied_for_taship.arn
  handler       = "lambda_function.lambda_handler"

  source_code_hash = filebase64sha256("../../student_request_for_taship/list_of_student_applied_for_taship.zip")
...
resource "aws_lambda_permission" "aws_lambda_list_of_student_applied_for_taship_permission" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.list_of_student_applied_for_taship.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn = "${aws_api_gateway_rest_api.ta_request_form.execution_arn}/*/*"
}
...
resource "aws_api_gateway_resource" "list_of_student_applied_for_taship" {
  rest_api_id = aws_api_gateway_rest_api.ta_request_form.id
  parent_id = aws_api_gateway_resource.ta_request_form.id
  path_part = "list_of_students_requested_for_ta"
}


# list_of_student_applied_for_taship : method
resource "aws_api_gateway_method" "list_of_student_applied_for_taship" {
  rest_api_id = aws_api_gateway_rest_api.ta_request_form.id
  resource_id = aws_api_gateway_resource.list_of_student_applied_for_taship.id
  http_method = "GET"
  authorization = "COGNITO_USER_POOLS"
  authorizer_id = aws_api_gateway_authorizer.ta_request_form.id
}

#  lambda integration : list_of_student_applied_for_taship-post-lambda
resource "aws_api_gateway_integration" "list_of_student_applied_for_taship-post-lambda" {
  rest_api_id = aws_api_gateway_rest_api.ta_request_form.id
  resource_id = aws_api_gateway_method.list_of_student_applied_for_taship.resource_id
  http_method = aws_api_gateway_method.list_of_student_applied_for_taship.http_method
  integration_http_method = "POST"
  type = "AWS_PROXY"
  uri = aws_lambda_function.list_of_student_applied_for_taship.invoke_arn
}


module "cors_list_of_student_applied_for_taship" {
  source  = "squidfunk/api-gateway-enable-cors/aws"
  version = "0.3.3"
  api_id          = aws_api_gateway_rest_api.ta_request_form.id
  api_resource_id = aws_api_gateway_resource.list_of_student_applied_for_taship.id
}

resource "aws_api_gateway_deployment" "ta_request_form_deployment" {
  depends_on= [aws_api_gateway_integration.student_request_for_taship-post-lambda,aws_api_gateway_integration.list_of_student_applied_for_taship-post-lambda]
  rest_api_id = aws_api_gateway_rest_api.ta_request_form.id
  stage_name  = "${var.env}"
}
